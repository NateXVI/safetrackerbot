class Bot {
	...
	async price(message, quantity) {
		...
		if (quantity) {
			message.channel.send(
				`Your **${this.comma(quantity)}** safemoon is worth **$${this.comma(
					data.usdLast * quantity
				)}**\nSource: ${data.exchangeName} at ${this.comma(data.usdLast)}`
			);
		} else {
			message.channel.send(
				'Gotta know how much you have first. Use the **set** command followed by the number to lemme know.'
			);
		}
	}
	...
	async rank(message) {
		...
		let output = '';
		for (let i in data) {
			output += `${Number(i) + 1}. ${data[i].exchangeName} at ${data[i].usdLast} ${
				data[i].url
			}\n`;
		}
	...
	async setStatus(client) {
		let data = await this.api.getOne();
		let price = data.usdLast;
		let places = 1000000000;
		price *= places;
		price = Math.floor(price);
		price /= places;
		client.user.setPresence({ activity: { name: `$${price}` } });
